...
/**
 * 成就徽章公開分享控制（ROADMAP #19 P3）。**沿用旅行地圖的同一把 `mapShareCode`**
 * （與年度回顧分享同套 getMapShareStatus / enableMapShare / disableMapShare），
 * 連結指向去識別化公開卡（/collections/share/[code]）——只露徽章與彙總數量，
 * 不含日期、航班號、飯店名或任何逐筆紀錄。因共用分享碼，文案明確標示與地圖是同一個開關。
 */
export function CollectionsShareDialog() {
  ...
  const shareUrl =
    code && typeof window !== 'undefined'
      ? `${window.location.origin}${ROUTES.COLLECTIONS_SHARE(code)}`
      : '';
  ...
  const handleCopy = async () => {
    try {
      ...
      logger.error('Failed to copy collections share link', err);
    }
  };
