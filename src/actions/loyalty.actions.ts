...
function toAccountItem(doc: LeanAccount): LoyaltyAccountItem {
  return {
    ...
    lifetime_diamond_years: doc.lifetimeDiamondYears ?? 0,
    lifetime_spend_usd: doc.lifetimeSpendUsd ?? 0,
    rollover_nights: doc.rolloverNights ?? 0,
    note: doc.note ?? '',
...
function toEntryItem(doc: LeanEntry): LoyaltyEntryItem {
  return {
    ...
    qualifying_stays: doc.qualifyingStays ?? 0,
    elite_qualifying_points: doc.eliteQualifyingPoints ?? 0,
      ...
      const doc = await LoyaltyAccount.findOneAndUpdate(
        { user: session.userId, program: parsed.data.program },
        {
          $set: {
            ...
            lifetimeDiamondYears: isHotel ? parsed.data.lifetime_diamond_years : 0,
            lifetimeSpendUsd: isHotel ? parsed.data.lifetime_spend_usd : 0,
            rolloverNights: isHotel ? parsed.data.rollover_nights : 0,
...
function entryFields(input: CreateLoyaltyEntryInput) {
  ...
  return {
    ...
    qualifyingStays: isHotel ? input.qualifying_stays : 0,
    eliteQualifyingPoints: isHotel ? input.elite_qualifying_points : 0,
