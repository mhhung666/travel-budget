...
export class NotePlanningError extends Error {
  constructor(
    public readonly code: 'FORBIDDEN' | 'NOT_FOUND' | 'VALIDATION_ERROR' | 'ACTIVITY_LIMIT'
  ) {
    super(code);
  }
}
...
export async function withNotePlanningTransaction<T>(
  db: mongo.Db,
  tripId: string,
  actorId: string,
  ...
  return db.client.withSession((session) =>
    session.withTransaction(
      async () => {
        ...
        if (!parent) throw new NotePlanningError('FORBIDDEN');
