...
export async function parseReceiptDraft(image: Buffer, mediaType: 'image/jpeg' | 'image/png' | 'image/webp'): Promise<ReceiptDraft> {
  ...
  const result = await generateText({
    ...
    output: Output.object({ schema: receiptDraftSchema, name: 'receipt_draft', description: 'Receipt expense draft' }),
    temperature: 0, timeout: Number(process.env.AI_RECEIPT_TIMEOUT_MS ?? 30_000), maxRetries: 1,
  });
