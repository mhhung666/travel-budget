...
### Travel map & sharing
The travel map ([src/app/[locale]/map/](src/app/%5Blocale%5D/map/), components in [src/components/map/](src/components/map/)) has four modes — routes (great-circle arcs), heat (leaflet.heat over itinerary-day `location`s), countries (choropleth), and photos (expense receipt images pinned via their linked itinerary-day coords — `getMapPhotos` + `groupPhotoPins`; login-only, receipts stay private via per-photo `getReceiptUrl`, never exposed on the public share). Notes that aren't obvious from the code:
