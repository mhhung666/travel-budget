...
- 兩端點已共用 MongoDB 持久化 global／user／trip 每日配額、成本預留與 token 結算，並具備授權、輸入、附件、配額及 provider 失敗的 route 測試。尚未具備 3B 非均分模式的自動帶入、產品事件，以及 fixture 品質評估，因此不得視為一般使用者可用或正式擴流。
