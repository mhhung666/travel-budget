...
**回饋**（2026-09-17）：花費分析的「平均每人每日」顯示 NT$62，同頁分攤是 NT$61.5；
...
- `dayCount` 先用旅行起訖日；沒有日期時退回「首筆到末筆支出日期」的天數。只有一筆支出就是 1，
  畫面沒有說明來源。

**建議做法**：

- 平均值改用 `roundMoney`（兩位小數），交給 `formatMoney` 顯示，與分攤一致（NT$61.5）。
...
- `avgPerPersonPerDay` 改用 `roundMoney`，QA 案例顯示 NT$61.5。
