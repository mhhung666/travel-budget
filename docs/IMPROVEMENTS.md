...
及整天欄位編輯；衝突保留草稿、更新查詢並提供四語提示。R2a 已保留既有活動 ID 並驗證身分，R2b 已將行程頁接上單筆原子寫入，R2c 已統一各入口的每日活動上限，R2d 已統一 revision；活動衝突粒度
與跨 collection transaction 仍待處理，見 [R 分階段進度](./ITINERARY_CONSISTENCY_PROGRESS.md)。

**問題**：行程頁已改單筆活動寫入，但衝突判斷仍是整天粒度；所有寫入入口已統一 revision，
