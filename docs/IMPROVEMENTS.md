...
| 2．R 行程與資料一致性 | R1～R4 工程完成；使用者回報 migration／push 完成；正式活動 CRUD、舊草稿衝突與 PDF 票券正常流程通過 | 部署與 migration／writer 同時上線證據；整天欄位修改、成員變更、跨 collection 競態；附件退休、R2 最終清理及 cron 結果 | [階段紀錄與部署要求](./ITINERARY_CONSISTENCY_PROGRESS.md)、[正式流程證據](./PRODUCTION_ACCEPTANCE_2026-09-12.md) |
