...
另已修正重載後補送失敗的摘要回復：以完整資料內容比對 optimistic 摘要，避免 JSON 還原後
物件參照不同而殘留筆數／金額；回歸涵蓋補送成功、失敗與保留後續更新的摘要。
本機另已完成傳輸中斷的安全重送：每次提交帶固定識別碼，支出與回應紀錄在同一交易寫入，
重送回傳原結果；pending 工作涵蓋傳輸中／重試中並可還原，舊佇列也可升級。
隔離 MongoDB 已驗證並發去重、交易回滾及刪除後不重建；實作、migration 與驗證界線見
[離線支出安全重送](./OFFLINE_EXPENSE_RETRY.md)。
尚待正式瀏覽器／IndexedDB／SW 重載驗收、送出前強制落盤、永久錯誤的草稿復原與離線載入失敗提示。
