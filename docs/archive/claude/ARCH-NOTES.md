...
> 讀者：要動對應子系統的 AI agent。**動哪節讀哪節**，不用全讀。
...
- Schema 定義在 [src/models/](../../../src/models)，Mongoose `autoIndex` 連線時建索引。可重現的索引/回填走 migrate-mongo（[docs/archive/details/MIGRATIONS.md](../details/MIGRATIONS.md)；`pnpm migrate:status|up|down|create`），與 autoIndex 並存（冪等、索引名對齊）。
...
- **相簿相片**（`photos/<tripId>/`，同私有 `receipts` bucket）：每張兩顆物件、**共用一個 uuid**（`<uuid>.jpg` 顯示＋下載／`<uuid>_t.webp` 縮圖），故簽名一次簽兩張（`createPhotoUploadUrls`）——分兩次呼叫會讓 `_t`／`_p` 無法從 key 推導。**顯示檔刻意自帶 GPS EXIF**（見 [FEATURES.md](../../FEATURES.md)），故公開路由絕不可簽它，只可簽由 [photoSanitize.ts](../../../src/lib/photoSanitize.ts) `ensureSanitizedPhotoCopies` idempotent 產生的消毒副本 `_p.jpg`。
...
- 主分頁只有四顆（行程／支出／相簿／結算），子分頁名稱固定為「每日行程／隨手記／清單」與「支出明細／花費分析」（花費分析＝單趟群組統計，URL 仍是 `/stats`；結算無子分頁）；分頁與子分頁**都定義在 [TripSpaceShell](../../../src/components/trips/space/TripSpaceShell.tsx) 的 `tabs`**，各頁自己不畫分頁列。`TRIP_DETAIL` 是所有子路由的前綴 → 比對一律 `exact`。
- `TripSpaceShell` 捲動 48px 後進 compact mode：名稱列縮短，非財務頁收起摘要，支出／花費分析／結算保留。行程落點的 [TripContextOverview](../../../src/components/trips/detail/TripContextOverview.tsx) 依 `getTripPhase` 把行前／旅中／旅後狀態縮成一列，並合併目的地、日期與可展開描述（原 TripHeader 已移除）。行程日期快速切換 [ItineraryDayNav](../../../src/components/trips/detail/itinerary/ItineraryDayNav.tsx) 黏在殼的頁首下方，靠 shell 以 ResizeObserver 寫入的 `--trip-space-header-height`（頁首高度會隨 compact mode 變）；日卡錨點一律用 `itineraryDayAnchorId`。
...
- 網頁入口與 PWA `/quick-add` 共用 [quickAdd.ts](../../../src/lib/quickAdd.ts) 的決策：排除封存，進行中優先；在行程空間內開啟時直接帶入目前旅行（須在我的旅行清單中，封存也算；AppShell 開啟當下由路徑取得）；否則唯一進行中／唯一 active 直開，多候選才 picker。`/quick-add` 頁本身不 redirect，由 AppShell 偵測 route 啟動同一 flow；不要再另寫一套 server 選擇邏輯。
- 全域導覽桌機／行動端都用旅行／地圖／統計／記一筆／我的分組；成就、回顧與設定歸「我的」。導覽事件只能經 [navigationEvents.ts](../../../src/lib/navigationEvents.ts) 的固定 taxonomy，不得附 trip/user/hash code、名稱、描述或金額。
...
- **計畫中／已造訪**：登入版地圖以 [isPlannedTrip](../../../src/lib/tripStatus.ts)（出發日當天起算已造訪；未排出發日算計畫中）區分。「今天」一律取瀏覽器本地日曆日：`useVisitedPlaces` 把 `today`（`YYYY-MM-DD`，也在 query key 裡）傳給 `getVisitedPlaces`，伺服器不用自己的時區判斷，前端目的地也用同一個值；只有沒帶或格式錯才退回伺服器時間。跨日後重新渲染時會依新日期重抓；沒有跨日計時器或返回頁面時的日期更新機制，持續閒置跨午夜不保證立即更新，本輪接受此限制。改旅行日期時 `useTripMutations` 會以前綴失效所有 `visitedPlaces` 查詢。國家／城市統計只算已造訪。**公開地圖 API 刻意不帶這個旗標**（會透露旅程是否已出發），公開頁仍全部當足跡計算。
