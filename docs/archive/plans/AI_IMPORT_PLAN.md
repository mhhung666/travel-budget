...
| Phase | 狀態 | 可交付結果 |
| --- | --- | --- |
| 3A | `in-progress` | 單張收據圖片解析成可編輯支出草稿（API、表單帶入、配額、40 張 fixture、evaluator 與產品觀測完成；尚待 provider 品質驗收） |
| 3B | `in-progress` | 單筆自然語言記帳與分攤草稿（API、唯一名稱與代名詞解析、四種分帳表單帶入、35 筆 fixture、evaluator 與產品觀測完成；尚待 provider 品質驗收） |
...
- 兩端點已共用 MongoDB 持久化 global／user／trip 每日配額、成本預留與 token 結算，並具備授權、輸入、附件、配額及 provider 失敗的 route 測試。3A 已有 40 張匿名合成圖片；3B 已有 35 筆合成文字 fixture，兩者皆具確定性離線評分與 opt-in live provider evaluator。`ai_expense_draft` 事件以固定 source／stage／result／error code 觀測開始解析、預覽、失敗與套用，不含輸入、草稿、名稱、ID、日期或金額。兩者尚未取得通過門檻的完整 provider 基線，因此不得視為一般使用者可用或正式擴流。
...
收據草稿已與行程匯入、文字記帳共用 `AI_PROVIDER` 與 provider credential；三種功能統一透過 `AI_MODEL` 選擇模型，使用 Vercel AI Gateway 時須選擇支援圖片輸入的模型，舊 OpenAI 直連部署仍相容。應抽取可共用的 AI provider 錯誤分類與 quota primitive，但不要在 Phase 3A 順便大幅重構已穩定的行程匯入。模型選擇不得寫死在文件；先用 fixture 比較支援圖片輸入的候選模型，再透過伺服器環境設定。
...
一般測試不呼叫 provider，只驗證圖片、預期草稿 schema 與評分器。`pnpm test:ai-receipt-eval` 才會逐張呼叫 live provider；可用 `AI_RECEIPT_EVAL_CASE_LIMIT` 限制樣本數、`AI_RECEIPT_EVAL_INTERVAL_MS` 控制間隔，後者預設 10 秒。評估器將 provider 可用率與成功輸出的欄位品質分開，並依共用 token 單價設定回報估算成本；完整 40 張基線仍須在不受 Free Tier 節流的環境執行。
...
- 第一人稱單數（`我`／`I`／`私` 等）可確定對應目前使用者；整團均分時，`大家`／`全員`／`我們`／`everyone`／`みんな` 等可展開為全員，`我和其他人` 亦視為全員。
- 單獨的 `其他人`／`others` 可能依賴未結構化的排除對象，不自動猜測；集體代名詞若出現在付款人、指定金額、百分比或份數中，同樣停在預覽要求修正。
- 日期超出旅程範圍、幣別缺漏、分攤不符時必須提供修正選項，不能直接寫入。

Phase 3B 已建立 35 份合成支出文字 fixture，涵蓋繁中、簡中、英文、日文與混合語言、多幣別、四種分帳、預設參與者、第一人稱／整團／不明指涉代名詞，以及幣別不明、同名、未知／重複成員與分攤不平衡等安全案例。離線 evaluator 將 schema 合法率與付款人、幣別、日期、分攤對象各自評分，參與者比較不受順序及 Unicode／大小寫差異影響。
