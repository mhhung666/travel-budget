...
09-11～09-12 正式站已驗證行程日新增／刪除、活動新增／修改／移除、兩分頁舊草稿衝突保護，
以及 PDF 票券上傳／保存／檢視。清理 cron 未授權請求回 401。
仍待核對部署 commit、migration／writer 一致上線、成員異動與跨 collection 競態、R2 最終清理及實際 cron 紀錄；
正常 UI 流程通過不代表 R 完整結案，詳見 [正式站補充驗收](../tests/PRODUCTION_ACCEPTANCE_2026-09-12.md)。
...
- `updateTrip` 在共用 Trip fence 交易重新驗證管理員與刪除狀態；部分日期編輯與交易內既有日期合併驗證，避免並行修改兩端形成倒置區間。
- 旅程欄位更新與日期觸發的 auto 相片重綁一起提交或回滾，沿用 helper 保留手動分類與自有 GPS。名稱／描述／目的地更新亦參與 fence；成功提交後才使路徑失效。
- 真實隔離 replica set 新增 5 項測試：重綁失敗回滾與重試、並行部分日期更新，以及初始驗權後降權／刪除中／已刪除拒絕。行程整合共 45 項通過。
...
- `deletePhotos` 在成員 Trip fence 交易內重新驗權、讀取相片與物件 key 並批次刪除，與旅程刪除及成員移除協調。保留一般成員可刪除、跨旅程隔離與部分 ID 命中的契約。
- R2 best-effort 清理與頁面失效在成功提交後執行；交易回滾不刪物件。
- 新增 6 項隔離 MongoDB 測試：一般成員／跨旅程／提交後清理、晚期失敗回滾與重試、移除／刪除中／已刪除，以及並行重複刪除。
...
- 筆記新增／修改／刪除、新增虛擬成員、好友批次加入與成員角色修改納入 Trip fence 交易。虛擬 User 與旅程成員一起建立或回滾。
...
R3 的應用程式資料一致性與清理重試已完成。AI 使用量暫存計數沿用到期／旅程延後清掃；外部通知及上傳採有限請求生命週期，已送出的 Email／Push 不可撤回，不涵蓋任意直接 DB／R2 修改或外部還原。R4 HEAD 有界平行查詢於後續 R4a～R4b 交付，見下節。
...
- R4a：新增驗證器及受控 Promise 測試；尚未接入 action，無產品行為變更。
- R4b：新增日、整天更新及單一活動新增／修改共同使用驗證器；每次 action 最多 4 個 HEAD，跨所有活動共用上限。此為單次請求上限，不是跨 instance 限流。
