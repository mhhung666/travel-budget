...
部署 commit 仍未核對，完整結果見 [補充驗收報告](../tests/PRODUCTION_ACCEPTANCE_2026-09-12.md)。以下工程驗證數字為原交付紀錄。
...
- 正式環境觀測移交 [改善建議 M](../plans/IMPROVEMENTS.md)，不再把 Q 列為待實作；AI 真實品質驗收仍依使用者要求暫緩。
...
- [x] 支出頁背景更新失敗保留內容與手動重試：09-15 桌面／手機 viewport 正式站通過，見 [補驗報告](../tests/PRODUCTION_ACCEPTANCE_2026-09-15.md)。
- [x] 09-16 相簿檢視器載入失敗重試與 23 筆支出列表／搜尋重設，在桌面／手機 viewport 通過，見 [補驗報告](../tests/PRODUCTION_ACCEPTANCE_2026-09-16.md)。
- [x] 09-16 既有圖片收據在桌面／手機 viewport 通過開啟、Escape 關閉及重新開啟；不含簽名過期或來源失敗，見同份補驗報告。
- [ ] 代表性大資料量與多批列表展開。09-16 另以既有 36 筆旅程通過兩種寬度的展開／搜尋／重設，仍未覆蓋超過 40 筆。
- [x] 09-16 留存 23 筆正式支出頁冷／熱、桌面／手機共 12 次 Network／Performance 證據，無 page error；不代表正式 CWV 或代表性負載通過。
- [ ] 依 [量測規範](../tests/QUERY_UX_PERFORMANCE.md) 補代表性負載與效能分析。

已新增單人臨時旅程的合成寫入驗收，未操作既有旅程資料或呼叫 AI provider。
正式站頁面 bytes／readyMs 小樣本已記錄；09-16 補存支出頁 Network／Performance trace，同日完成 Network 等待分析，代表性負載與伺服器瓶頸歸因尚未完成。
...
- Q2 production build 與 bundle 比較已完成（見下）；Q3 已完成本機 Network／Performance 量測。
  正式站少量冷／熱混合樣本不作為改善基準；完整條件與限制見 [效能報告](../tests/QUERY_UX_PERFORMANCE.md)。
...
- 預載、server hydration、cursor pagination 決策及重跑方式見 [QUERY_UX_PERFORMANCE.md](../tests/QUERY_UX_PERFORMANCE.md)。
...
- 最終 production build 通過；36 次隔離 browser 載入全部成功（desktop／mobile／worker × 首頁／支出 × cold／warm × 3），
  沒有 page error 或未預期 API／寫入。結果、日期／Chrome 版本差異與未改善指標均記入 [效能報告](../tests/QUERY_UX_PERFORMANCE.md)。
