...
> 更新日期：2026-09-15（整理驗證範圍；未新增真人測試結果）
> 介面規則的權威來源是 [UI_UX_SPEC.md](../details/UI_UX_SPEC.md)。本文件只記目前狀態與尚未完成的驗證。
...
- 全域主要入口固定為旅行、地圖、統計、記一筆、我的；桌機與行動端維持同一心智模型。
...
完整可檢查規則見 [UI_UX_SPEC.md](../details/UI_UX_SPEC.md)。
...
09-15 使用者決定 iOS Safari／安裝版 PWA 暫時通過、尚未實測，本輪不列為阻擋。
此範圍調整不等於上表的真人任務、輔助科技或其他色彩對比驗證完成；目前整體進度見 [專案總覽](../../README.md)。

## Agent 驗收入口

1. 介面變更先核對 [UI_UX_SPEC.md](../details/UI_UX_SPEC.md)。
