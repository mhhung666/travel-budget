...
**已結案（2026-09-16 部署）。** 五輪本機與實際 MongoDB 驗收的缺口皆已修正；正式庫完成舊金額 migration，正式站確認「成都重慶」預算列顯示 NT$2,043；編輯外幣支出回存的精度問題經瀏覽器重現、修正並部署。結案摘要見文末〈結案〉，以下各輪為當時紀錄，其中「待確認」字樣以結案段落為準。
...
`migrations/20260916220000-normalize-expense-money.js` 把讀取端的取整與尾差分配寫回資料庫：
冪等 `up`／`down`，原值備份於 `expense_money_migration_backups`，更新條件含原值（遷移中被編輯的
支出略過），`down` 只回復仍等於遷移結果的支出。規則凍結一份副本，由
`expenseMoneyMigration.test.mjs` 對照 `money.ts` 驗證等價；`expenseMoneyMigration.integration.test.mjs`
在 CI money job 以真實 MongoDB 驗證讀取結果不變、重跑、回復與並行編輯。

正式庫：套用前唯讀試算 126 筆中 53 筆會更新（金額變動 < 半分、分攤變動 ≤ 一分、異常 0）；
套用後複查 126 筆全為整分、再跑不變更任何一筆、異常 0、備份 53 筆。其他環境需 `pnpm migrate:up`。

### 正式站

正式庫逐趟逐成員查核預算、結算與統計 0 筆不一致。部署後「成都重慶」行程頁顯示
「我的花費 NT$2,043、剩餘 NT$17,957」（預算 NT$20,000；4 筆支出、兩位成員各應分攤 2,043），
與資料庫計算一致。

### 編輯回存

本機瀏覽器（Playwright + Chrome，獨立測試庫）重現兩個問題：

- 編輯表單逐筆以「TWD ÷ 匯率」反算原幣並取位數。TWD 只存到分，低匯率幣別誤差放大：
  非均分支出未改任何欄位即無法儲存（模擬 KRW 約九成、VND 約 98%）；JPY 取整造成約 17% 無法儲存、
  約 38% 回存後分攤變動數分。
- 分攤金額輸入框未設 `step`，瀏覽器擋下 50.01 等小數，新增與編輯都無法送出。

修正（`987219d`，4.6.55）：`reconstructOriginalShares` 以原幣總額按 TWD 分攤比例用最大餘數法分配，
只在與均分結果完全相同時推回均分模式；輸入框加 `step="any"`。模擬 7 幣別 × 21,000 例阻擋 0、
分攤變動 0；瀏覽器 KRW／JPY／USD／TWD 支出開啟後直接儲存與修改描述後儲存，分攤皆不變，
小數指定金額可新增。非此表單算出的舊分攤回存時各人可能差至多一分。

### 後續（不影響結案）

- 資料已正規化，`tripShellRead` 與統計聚合每次讀取的尾差重算可移除並比較查詢成本（效能項目）。
- 表單內 TWD 分攤試算以整數顯示（58.53 顯示 NT$59），儲存值不受影響。
