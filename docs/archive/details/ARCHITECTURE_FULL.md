...
- **全域資訊架構**：桌機與行動端同為旅行／地圖／統計／記一筆／我的；成就、會籍、年度回顧與設定歸入「我的」。[navigationEvents.ts](../../../src/lib/navigationEvents.ts) 的 `navigation_used` 只接受固定 `target`／`surface` union，不得加入動態 id、邀請碼、名稱、描述或金額。
