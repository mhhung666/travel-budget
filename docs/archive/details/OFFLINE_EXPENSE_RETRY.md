...
安全重送於 2026-09-12 完成；S 的獨立本機保存、草稿復原與真實瀏覽器驗收見
[分階段完成紀錄](../history/OFFLINE_EXPENSE_COMPLETION.md)。
...
- 傳輸拒絕與 `INTERNAL_ERROR` 保留 pending 工作及暫存列，間隔由 1 秒遞增至最多 30 秒，連線可用時繼續重試。
  權限、驗證及請求內容衝突等明確失敗保存為可編輯／匯出的草稿，不自動重試。
- IndexedDB 的獨立寄送紀錄保存 paused、傳輸中及等待重試的新增支出；還原為 paused 後再交給 QueryClient 補送。
...
一般讀取快取仍有 1 秒節流與 7 天有效期；獨立寄送紀錄不受該節流、期限或快取格式更新影響。
表單先等待 IndexedDB transaction 完成才顯示暫存成功，儲存失敗保留表單且不送出。
永久錯誤保留草稿，詳見完整 S 驗收紀錄。

使用者明確登出並同意捨棄、清除網站儲存空間，或瀏覽器自行移除網站資料時，本機紀錄無法繼續保存。
舊伺服器不識別新欄位，部署或回滾時不可把舊伺服器視為支援安全重送。
