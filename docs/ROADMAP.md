...
> **進度**：Tier 1 全數完成 — **#1 預算**、**#2 結算「標記已付」**、**#3 彈性分帳**皆已實作並合併。下一波請見 Tier 2（建議自 #4 收據／#13 群組統計 起手）。
...
**Tier 1 已全數完成**（#1 預算、#2 結算閉環、#3 彈性分帳）。下一波建議從無需新基礎設施的 **#13 群組統計**（純查詢層改動）起手，或一次性導入 blob 儲存解鎖 **#4 收據／#11 頭像**。#9 通知可接續 #2 做「結算提醒」。
