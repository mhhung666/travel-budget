...
### 9. ⭐ 通知 (Notifications) — L（需基礎設施）〔Phase 1 站內通知 ✅ 2026-06-28；Phase 2a Email ✅ 2026-06-28；Web Push / 排程提醒待做〕
...
>
> **做法（Phase 2b / 3 待做）**：排程結算提醒（Vercel Cron 每週彙整未結清 → 寄 Email，需受保護的 cron route + `CRON_SECRET` + `vercel.json`，部署後生效）、Web Push（VAPID + `PushSubscription` model + service worker，建議與 #5 離線優先共用 SW）。輪詢式 badge 屆時可換成 SSE / Web Push 即時推送。
...
| **Email / 排程** | #9 Email 通知（已解鎖）、結算提醒、邀請信 | **Resend**（已採用，env-gated）+ Vercel Cron（排程待做） |
...
第三波（協作與留存）← 進行中
  ├── 9  通知      ✅ Phase 1 站內通知 + 鈴鐺 ・ Phase 2a Email（Resend）　｜　Phase 2b 排程提醒・Web Push 待做
...
**下一步建議**：#9 Phase 2a Email 基礎層已完成（Resend，env-gated）。接續可（a）做 **#9 Phase 2b**（Vercel Cron 每週排程結算提醒，需部署 + `CRON_SECRET`）或 **Phase 3 Web Push**（建議與 #5 共用 service worker）；或（b）做 **#5 離線優先 PWA**（旅行殺手級體驗）或 **#15 年度回顧**（傳播）；期間可穿插 S 級填空（#12 旅伴、#16 地圖統計、#18 標籤）。
