...
### 16. 🔹 地圖相片釘點 (Map photo pins) — M〔統計儀表板已完成，相片釘點待做〕
**為什麼**：地圖已有三模式 + 按天花費熱點權重 + 統計儀表板，但缺相片情境。
**做法**：（連動 #4）把支出收據附件（R2）依所在地點釘上地圖、點擊看照片。屬跨功能整合（座標關聯 + 私有 presigned GET），非純前端聚合，成本 M。可選延伸：更深的統計洞察（各國/城市造訪次數排行、每趟里程排行）。
