...
### UI/UX 第二輪 Phase 2B：全域快速記帳（2026-07-27）

- 桌機頂列與行動底部導覽加入全域「記一筆」；行動個人統計移至「我的」，旅行內重複的新增支出 FAB 移除。
- 單一進行中／唯一可用旅行直接開金額表單，多旅行使用進行中／最近 picker 並記住上次選擇；無旅行時接續建立旅行與第一筆支出。
- 網頁入口與 PWA `/quick-add` 共用選擇流程；表單在持續存在的 App Shell 開啟，不以換頁破壞原頁篩選與捲動狀態。完整規則與驗證見 [UI_UX_EVALUATION.md](./UI_UX_EVALUATION.md)。

### UI/UX 第二輪 Phase 3：資訊架構與旅行首頁（2026-07-27）

- 桌機／行動端統一為「旅行／地圖／記一筆／我的」分組，低頻個人資料收進「我的」；新增只含固定分類的隱私安全導覽事件基準。
- 旅行 sticky header 加入 compact mode，非財務頁捲動後收起金額摘要；子分頁改為「每日行程／結算方案／群組統計」，旅行資訊收成可展開 cover。
- 行程落點依行前／旅中／旅後顯示倒數與待辦、Day N 與今日活動／花費、待結算與回顧入口。
- 加入對話框接受邀請 URL 或代碼，公開邀請頁補旅行脈絡與登入返回，加入成功直接進旅行；旅行卡的邀請碼移入操作選單。完整交付與驗證見 [UI_UX_EVALUATION.md](./UI_UX_EVALUATION.md)。

### UI/UX 第二輪 Phase 4：自動化驗證與量測基線（2026-07-27）

- 導入 axe-core 與真實 render／user-event 測試，涵蓋空旅行、首次行動、旅行資訊展開、行動導覽、快速記帳觸發與完整邀請 URL 加入流程；提供 `pnpm test:a11y` 快速驗證入口。
- 建立固定型別、best-effort 的隱私安全事件 taxonomy，接上註冊、建旅、邀請同行、分享邀請、首筆支出、快速記帳路徑、錯帳修正 bucket 與離線 queued／synced／failed；不送 id、名稱、描述、邀請碼、日期、位置或精確金額。
- 完成 [UI/UX 實作規格](./UI_UX_SPEC.md) 與 [5–7 人可用性測試套件](./USABILITY_TEST_PHASE4.md)。自動化 code baseline 已完成；真人測試與正式環境指標基準仍待外部執行，未標示為完成。
