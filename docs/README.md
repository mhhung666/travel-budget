...
**S 離線支出缺陷已修正並通過正式站複驗；Q、O、P、R 的工程交付已完成，但部分正式環境驗收仍未結案。**
...
1. **既有工程驗收**：依 O → P → R → M 的既定優先順序收尾；各項缺少的證據與完成條件集中在 [IMPROVEMENTS.md](./IMPROVEMENTS.md)。外部條件未齊時，可先做 M 尚未完成的 UI 情境。
