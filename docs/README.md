...
| 分類 | 項目 | 目前結論 | 詳細資料 |
| --- | --- | --- | --- |
| 已完成修正與複驗 | S：離線支出保存、補送與摘要一致性 | 09-14～09-15 正式站通過；分階段失敗後兩次離線重載摘要正確 | [正式站複驗](./PRODUCTION_ACCEPTANCE_2026-09-14.md#09-15-摘要時序複驗) |
...
| 已驗收結案 | P：支出背景處理 | 09-16 iPhone 推播與站內通知通過；使用者接受其餘項目免驗結案，保留證據界線 | [P 結案紀錄](./EXPENSE_DELIVERY_ACCEPTANCE.md) |
...
| 暫緩 | AI 真實 provider 品質／真人效益 | 已有實作基線；不列入目前排程 | [AI 狀態與候選功能](./ROADMAP.md) |

## 接下來要做什麼

1. **既有工程驗收**：依 O → R → M 的既定優先順序收尾；各項缺少的證據與完成條件集中在 [IMPROVEMENTS.md](./IMPROVEMENTS.md)。外部條件未齊時，可先做 M 尚未完成的 UI 情境。
2. **尚未實作的改善**：A／I／J 需要方案與實作；G／H 先觀察實際資料量與快取使用，再決定是否做。這些不是已確認的驗收失敗。
3. **其他驗證與新功能**：UI/UX 依專屬清單排程；OAuth、PDF 報告、逐項分帳等是 [ROADMAP.md](./ROADMAP.md) 候選，不是本輪修正的漏項。
...
| 紀錄 | 用途 |
| --- | --- |
| [09-08 MongoDB 線上抽查](./MONGODB_LIVE_ACCEPTANCE.md) | 登入、session 與唯讀流程證據，非正式負載量測 |
