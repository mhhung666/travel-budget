...
更新：2026-09-15。應用程式版本以 package.json 為準。

## 目前驗收狀態（2026-09-15）

**P 工程已完成，正式環境驗收尚未結案。** 跨項目摘要見 [專案總覽](./README.md)。
...
- 待驗：部署、開關、實際排程、DTO／逐事件 checkpoint、收件人通知與推播、隔離失敗恢復及延遲量測。曾出現一次 inspect 503，原因待 trace 追查。

唯一詳細勾選表見下方「正式結案紀錄」。以下工程交付段落的「本輪未部署」等敘述為 09-08 當時界線，非目前部署結論。
...
- [x] 單人測試旅程支出 CRUD、活動紀錄及工作 done 計數（09-11～09-12）。
- [ ] 補齊 DTO、逐事件 checkpoint、其他收件人站內通知與實際推播。
...
上述未勾選項目仍缺完整證據，P 不標成正式完成；部署狀態需以平台紀錄核對。
